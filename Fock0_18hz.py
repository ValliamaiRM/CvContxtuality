...
def interaction_diftime(w1,w2,lam1,lam2,d1,d2,ini_state,t_in1,t_in2,tau):
    ...
    def H_int11_func(t):
        return lam1 * np.exp(-1j * (w1 - d1) * t) * np.exp(1j * w1 * t)

    def H_int21_func(t):
        return lam1.conjugate() * np.exp(1j * (w1 - d1) * t) * np.exp(-1j * w1 * t)

    def H_int12_func(t):
        return lam2 * np.exp(-1j * (w2 - d2) * t) * np.exp(1j * w2 * t)

    def H_int22_func(t):
        return lam2.conjugate() * np.exp(1j * (w2 - d2) * t) * np.exp(-1j * w2 * t)
...
# Free evolution 
def FreeEvolution(state,start,stop):
    H_free = Qobj(tensor(qeye(2),qeye(N),qeye(N)))
    time = np.linspace(start,stop,n)
    out_state = mcsolve(H_free,state,time,c_ops, ntraj=ntraj,options={"keep_runs_results": True,"atol":1e-5, "rtol":1e-3,"method":'adams',"norm_tol":1e-3,"progress_bar":False})
    return Qobj(out_state.runs_states[0][-1])#, dims = [[2, N, N], [2,N,N]])
...
initial_state = tensor(basis(2,1),coherent_state1,coherent_state2)
...
# 15Hz gama dephasing noise
gama = 18
c1 = tensor(qeye(2), destroy(N).dag()*destroy(N),qeye(N))
c2 = tensor(qeye(2),qeye(N), destroy(N).dag()*destroy(N))
c_ops = [np.sqrt(gama) * c1, np.sqrt(gama) * c2]
R1=[]
R2=[]
R3=[]
C1=[]
...
for i in range(500):
  ...
  np.random.seed(seed)  # Set the seed for NumPy RNG
...
with open("Fock0_18hz.csv", "a", newline="") as f:
    csv.writer(f).writerows([R1, R2, R3, C1, C2, C3, chi])
